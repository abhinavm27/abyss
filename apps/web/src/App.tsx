...
function VelaAuth({ onAuthenticated }: { onAuthenticated: () => void }) {
  ...
  const demo = async () => {
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : "VELA could not open the demo account."); }
    finally { setBusy(false); }
  };
  return <main className="vela-auth"><section><span className="vela-logo">VELA</span><p>Your clearest path to care.</p><h1>{create ? "Create your secure account." : "Welcome back."}</h1><button className="vela-demo-login" disabled={busy} onClick={() => void demo()}>Open demo account</button><div className="vela-auth-divider"><span>or use an account</span></div><form onSubmit={(event) => { event.preventDefault(); void submit(); }}><label>Email<input type="email" autoComplete="email" required value={email} onChange={(event) => setEmail(event.target.value)} /></label><label>Password<input type="password" autoComplete={create ? "new-password" : "current-password"} minLength={8} required value={password} onChange={(event) => setPassword(event.target.value)} /></label>{error && <div role="alert">{error}</div>}<button disabled={busy}>{busy ? "Connecting…" : create ? "Create account" : "Continue securely"}</button></form><button className="vela-auth-switch" onClick={() => setCreate((value) => !value)}>{create ? "I already have an account" : "Create an account"}</button><small>The pricing and care-journey engine are real. Every consequential action (enrollment, booking) is sandboxed and requires your explicit approval.</small></section></main>;
}
