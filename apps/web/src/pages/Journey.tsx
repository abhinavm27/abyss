    ...
    <main className="mx-auto min-h-dvh w-full max-w-md bg-background px-5 pb-24 pt-8">
      ...
      {!snapshot ? (
        ...
        <>
          ...
          {snapshot.hospital_rates.length > 0 && <section className="mt-5 space-y-3"><div><h2 className="font-medium text-foreground">Published hospital rates</h2><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Read directly from hospital machine-readable files. Network status is unknown; these are not guaranteed out-of-pocket prices.</p></div>{snapshot.hospital_rates.slice(0, 3).map((rate) => <div key={rate.hospital_id} className="rounded-xl border border-border bg-card p-4"><div className="flex justify-between gap-3"><b className="text-sm">{rate.hospital}</b><b className="text-sm">${rate.typical.toLocaleString()}</b></div><p className="mt-2 text-xs text-muted-foreground">Published range ${rate.low.toLocaleString()}–${rate.high.toLocaleString()} · {rate.rate_count} rate{rate.rate_count === 1 ? "" : "s"} · updated {rate.source.published_at ?? "unknown"}</p>{rate.source.source_page_url && <a className="mt-2 inline-block text-xs text-primary underline" href={rate.source.source_page_url} target="_blank" rel="noreferrer">Hospital source</a>}</div>)}</section>}
